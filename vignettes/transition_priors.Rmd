...
# MultinomialCI is in Suggests; if it is not installed (e.g. on a platform
# where the CRAN binary is temporarily unavailable), fall back to an empty
# data frame so the plot still renders without the multinomial CI overlay.
if (requireNamespace("MultinomialCI", quietly = TRUE)) {
  ...
  message("MultinomialCI not installed; rendering plot without multinomial CI overlay.")
  mnCI <- tibble(
    interval_width = character(0),
    lcl = numeric(0),
    ucl = numeric(0),
    p = numeric(0),
    prior = factor(character(0), levels = c("None", "Uninformative", "RLT, w = 1", "RLT, w = 0.5N", "RLT, w = N")),
    transition = factor(character(0), levels = c("s to s", "s to j", "s to a", "s to m"))
  )
}
