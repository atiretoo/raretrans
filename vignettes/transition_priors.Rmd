...
knitr::opts_chunk$set(fig.width = 5, fig.height = 5, fig.path = "images/")
```
...
```{r}
desc_stats_priors <- diffPriors %>% 
  group_by(prior) %>% 
  summarize(mean = mean(lprior),
            sd = sd(lprior),
            min = min(lprior),
            max = max(lprior))
desc_stats_raw <- diffPriors %>% 
  filter(prior == "Uniform") %>% 
  summarize(mean = mean(lmbda),
            sd = sd(lmbda),
            min = min(lmbda),
            max = max(lmbda))
knitr::kable(bind_rows(desc_stats_raw, desc_stats_priors), digits = 3)
```
